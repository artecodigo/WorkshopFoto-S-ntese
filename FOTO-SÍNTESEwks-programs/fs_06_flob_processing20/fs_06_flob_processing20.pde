...
boolean ENVIA = false;
int  valor_tempo = 2;
int  valor_frequencia = 440;
int  valor_frequencia_byte = 440 / 4;
...
void draw() {
  ...
  if(ENVIA){
     ...
     valor_tempo = (int) map( flob.getPresencef(), 0,1, 1, 10 ) ;
     
     
     // procurar a blob maior
     int qual = -1;
     float areamax = -1;
     for(int i = 0; i < flob.getNumBlobs(); i++) {
        ABlob ab = (ABlob)flob.getABlob(i);     
        float area = ab.dimx * ab.dimy;        
        if( area > areamax) {
           areamax = area;
           qual = i; 
        }    
     ...
     if(qual > -1) {
         ABlob ab = (ABlob)flob.getABlob(qual);   
         
         valor_frequencia = (int) map( ab.cy, 0,height, 2050, 10 ) ;
         valor_frequencia_byte = valor_frequencia / 4;
         
         // enviar a freq + tempo
         myPort.write( valor_frequencia_byte );
         myPort.write( valor_tempo );
     }      
