...
void draw(){
 ...
 //   float angulo = frameCount * 0.25;
    float angulo = frameCount * 
                    map(mouseX,0,width, 0.005, 1   );
  
   led0 = (int) map( sin(angulo), -1,1, 0, 100);
   led1 = (int) map( sin(angulo + PI/2), -1,1, 0, 100);
   led2 = (int) map( sin(angulo + PI), -1,1, 0, 100);
...
//   led2 = (int) map( sin(frameCount*0.505), -1,1, 0, 100);
 ...
 // assegurar que o boot do serial 100%
 if(frameCount>100){
